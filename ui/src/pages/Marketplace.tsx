...
export function Marketplace() {
  ...
  const showIntegrations = activeCategory === "integration";
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Integrations tab — unified provider management */}
      {showIntegrations && selectedCompanyId && (
      ...
      {showIntegrations && !selectedCompanyId && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Choose a company from the sidebar to manage its integrations.
          </p>
        </div>
      )}

      {/* Loading State */}
      {!showIntegrations && isLoading && (
      ...
      {/* Empty State */}
      {!showIntegrations && !isLoading && data && data.items.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <Store className="h-12 w-12 text-muted-foreground/30 mb-4" />
      ...
      {/* Items Grid */}
      {!showIntegrations && data && data.items.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {data.items.map((item) => (
            <MarketplaceCard
              key={item.id}
      ...
      {/* Results count */}
      {!showIntegrations && data && data.total > 0 && (
